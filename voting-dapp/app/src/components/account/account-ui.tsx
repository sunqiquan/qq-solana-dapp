...
function ModalVote(props: { address: Address }) {
  const mutation = useVoteMutation({ address: props.address })
  const [pollId, setPollId] = useState(3)
  const [voteChoice, setVoteChoice] = useState<'Smooth' | 'Crunchy'>('Smooth')
  ...
  return (
    <AppModal title="Vote" submitDisabled={!pollId || !voteChoice} submitLabel="Send" submit={handleSubmit}>
      {/* select box */}
